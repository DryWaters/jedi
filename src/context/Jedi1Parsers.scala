...
class Jedi1Parsers extends RegexParsers {
  ...
  def equality: Parser[Expression] = inequality ~ rep("==" ~> inequality) ^^ {
    case p ~ Nil  => p
    case p ~ more => FunCall(Identifier("equals"), p :: more)
  }

  // inequality ::= sum ~ (("<" | ">" | "!=") ~ sum)?
  def inequality: Parser[Expression] = sum ~ opt(("<" | ">" | "!=") ~ sum) ^^ {
    case exp1 ~ None              => exp1
    case exp1 ~ Some("<" ~ exp2)  => FunCall(Identifier("less"), exp1 :: exp2 :: Nil)
    case exp1 ~ Some(">" ~ exp2)  => FunCall(Identifier("more"), exp1 :: exp2 :: Nil)
    case exp1 ~ Some("!=" ~ exp2) => FunCall(Identifier("unequals"), exp1 :: exp2 :: Nil)
  }
  ...
  def operands: Parser[List[Expression]] = "(" ~> opt(expression) ~ opt(rep("," ~> expression)) <~ ")" ^^ {
    case Some(expression) ~ None          => expression :: Nil
    case Some(expression) ~ Some(moreExp) => expression :: moreExp
    case _ => Nil
  }
